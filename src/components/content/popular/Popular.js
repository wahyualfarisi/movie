...
class Popular extends Component {
  ...
  _loadAllPopular = async () => {
    // func load all popular with parse query string in parameter
  ...
  render() {
    ...
    if (popular === null || isLoaded) {
      ...
      if (Object.keys(popular).length > 0) {
        ...
        displayContent = <h1>Terjadi Masalah</h1>;
      }
    }
      ...
      <div className="android-card-container mdl-grid">
        <div className="android-more-section">
          <div className="android-section-title mdl-typography--display-1-color-contrast">
            {isLoaded ? "" : "Populars"} <br />
            {PageInfo}
          </div>

          <div className="wrap">{displayContent}</div>
...
export default connect(
  mapStateToProps,
  { getPopularMovie, getLoadMoreMovie }
)(Popular);
